...
import { extractDataFromImages, AVAILABLE_MODELS, DEFAULT_MODEL } from './utils/visionApi'

function App() {
  const { products, addProduct, updateProduct, removeProduct, clearProducts } = useProductStore();
  ...
  const [model, setModel] = useState(() => localStorage.getItem('mercado_app_model') || DEFAULT_MODEL);
  ...
  useEffect(() => {
    localStorage.setItem('mercado_app_model', model);
  }, [model]);
  ...
  const handleCapture = async (images) => {
    ...
    try {
      const extracted = await extractDataFromImages(images, apiKey, model);
            ...
            <div className="space-y-4">
              <div className="space-y-1">
                ...
                <p className="text-[9px] text-gray-400 mt-1 italic">
                  *Obrigatória — obtenha em aistudio.google.com
                </p>
              </div>

              <div className="space-y-1">
                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest pl-1">Modelo de IA</label>
                <select
                  value={model}
                  onChange={(e) => setModel(e.target.value)}
                  className="w-full bg-gray-100 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 appearance-none"
                >
